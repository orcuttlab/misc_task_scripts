...
https://github.com/edgraham/PhylogenomicsWorkflow/blob/master/identifyHMM
...
parser.add_argument('-E',help='Set highest E-Value to be considered for a HMMsearch match. Default: 1E-5',default='1E-5')
...
for faa in fpaths:
    ...
    for hmm in hpaths:
    	...
    	if arg_dict["cut_tc"] == "True":
			subprocess.call(["hmmsearch","--cut_tc","--tblout", str(str(gname)+"_"+str(hname)+".CRISPR.txt"), "--notextw", hmm, str(faa)],stdout=hmmer_log)
    	else:
    		subprocess.call(["hmmsearch","-E",Evalue,"--tblout", str(str(gname)+"_"+str(hname)+".CRISPR.txt"),"--notextw", hmm, str(faa)],stdout=hmmer_log)
